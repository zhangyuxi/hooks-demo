...
function getInputGroup(children) {
	return React.createElement.apply(null, ['div', {
...
function create(overrides = {}) {
	function textbox(locals) {
		locals.config = textbox.getConfig(locals); // eslint-disable-line no-param-reassign
		locals.attrs = textbox.getAttrs(locals); // eslint-disable-line no-param-reassign
	...
	textbox.renderInput = overrides.renderInput || function renderInput(locals) {
		return locals.attrs.noteditable ? <Text className={locals.attrs.className}>{locals.value}</Text> : <Input {...locals.attrs} />;
	};

	textbox.renderTextarea = overrides.renderTextarea || function renderTextarea(locals) {
		return <Input.TextArea {...locals.attrs} />;
	};
