...
function getBreakpoints(breakpoints) {
  ...
  for (const size in breakpoints) {
    if (breakpoints.hasOwnProperty(size)) {
      className['ant-col-' + size + '-' + breakpoints[size]] = true
    }
  }
  return className
}
...
function create(overrides = {}) {
  ...
  list.renderRowButton = overrides.renderRowButton || function renderRowButton(button) {
    return <button key={button.type} type="button" className={`ant-btn ant-btn-default ant-btn-${button.type}`} onClick={button.click}>{button.label}</button>
  }

  list.renderButtonGroup = overrides.renderButtonGroup || function renderButtonGroup(buttons /* , locals*/) {
    return <div className="ant-btn-group ant-btn-group-sm">{buttons.map(list.renderRowButton)}</div>
  }
  ...
  list.renderAddButton = overrides.renderAddButton || function renderAddButton(locals) {
    ...
    return (
      <div className="row">
        <div className="ant-col-lg-12">
          <div style={{marginBottom: '15px'}}>
            <button type="button" className={`ant-btn ant-btn-default ant-btn-${button.type}`} onClick={button.click}>{button.label}</button>
          </div>
        </div>
      </div>
    )
